...
class ParsingConfidenceCalculator:
    ...
    def calculate_confidence(
        self, students: List[StudentRecord], metadata: PDFMetadata, raw_text: str
...
class StudentBlockParser:
    ...
    def parse_student_block(self, block: str) -> Optional[StudentRecord]:
        ...
        total_marks = sum((sub.total or 0) for sub in subjects if not sub.is_ac)
...
class ResultParser:
    ...
    ) -> Tuple[PDFMetadata, List[StudentRecord], float, List[str], str]:
        """
    ...
    def parse_pdf_bytes(
        self, pdf_bytes: bytes, progress_callback=None
    ) -> Tuple[PDFMetadata, List[StudentRecord], float, List[str], str]:
