...
def _generate_excel(metadata, students) -> bytes:
    ...
    def style_ws(ws, df):
        ...
        for col_num, col_name in enumerate(df.columns, start=1):
            ...
            max_len = max(len(str(col_name)),
                *[len(str(ws.cell(row=r, column=col_num).value or "")) for r in range(2, ws.max_row + 1)], 0)
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        df1 = pd.DataFrame([{
            "PRN": s.prn, "Seat No": s.seat_no, "Name": s.name,
            "SGPA": s.sgpa, "Credits Earned": s.credits_earned,
            "Credits Total": s.credits_total, "Subjects": len(s.subjects),
        ...
        df2 = pd.DataFrame([{
            "Rank": i, "PRN": s.prn, "Seat No": s.seat_no, "Name": s.name,
            "SGPA": s.sgpa, "Status": "PASS" if (s.sgpa or 0) >= 4.0 else "FAIL",
            "Category": _sgpa_category(s.sgpa),
        ...
        for code, entries in sorted(subj_map.items()):
            ...
            subj_rows.append({
                "Subject Code": code, "Appeared": appeared,
                "Passed": passed, "Failed": appeared - passed,
        ...
        labels = ["<4 (Fail)", "4-5 (Pass)", "5-6 (Second)", "6-6.75 (Higher Second)",
                  "6.75-7.75 (First Class)", "7.75-8.5 (Distinction)", ">8.5 (Outstanding)"]
        sgpas = [s.sgpa for s in students if s.sgpa is not None]
        dist = {lbl: 0 for lbl in labels}
        for sgpa in sgpas:
            for i in range(len(bins) - 1):
                if bins[i] <= sgpa < bins[i + 1]:
                    dist[labels[i]] += 1; break
            else:
                dist[labels[-1]] += 1
        ...
        df5 = pd.DataFrame([
            ("University", metadata.university_name), ("College", metadata.college_name),
            ("Department", metadata.department_name), ("Semester", metadata.semester_number),
            ...
            ("Pass Count", pass_count), ("Fail Count", len(students) - pass_count),
...
def render():
    ...
    cards = [
        ...
        (c5, "#a78bfa", "📅 Session",    f"{metadata.session_type} {metadata.session_year}"),
    ]
    ...
    if students:
        ...
        preview_rows = [{
            "PRN": s.prn, "Seat No": s.seat_no, "Name": s.name,
            "SGPA": s.sgpa, "Subjects": len(s.subjects),
            "Status": "✅ PASS" if (s.sgpa or 0) >= 4.0 else "❌ FAIL",
    ...
    with save_col:
        save_clicked = st.button(
            "💾 Save to ResultOps Database", type="primary", use_container_width=True,
        )
    with note_col:
